...
class LandingPageServiceSuite extends CatsEffectSuite {
  private lazy val dataset1 = new MemoizedDataset(
    Metadata("id" -> "id1", "title" -> "title1"),
    null,
    null
  )
  private lazy val dataset2 = new MemoizedDataset(
    Metadata("id" -> "id2", "title" -> "title2"),
    null,
    null
  )
  private lazy val latisInterp =
    new Latis3Interpreter(Catalog(dataset1, dataset2))
  private lazy val landingPageService =
    new LandingPageService[IO](latisInterp)
